...
TEST_F(BraveShieldsUtilTest, GetPatternFromURL) {
  ...
  pattern = GetPatternFromURL(GURL("http://nixbrowser.in"));
  EXPECT_TRUE(pattern.Matches(GURL("http://nixbrowser.in")));
  EXPECT_TRUE(pattern.Matches(GURL("http://nixbrowser.in/path1")));
  EXPECT_TRUE(pattern.Matches(GURL("http://nixbrowser.in/path2")));
  EXPECT_TRUE(pattern.Matches(GURL("https://nixbrowser.in")));
  EXPECT_TRUE(pattern.Matches(GURL("ftp://nixbrowser.in")));
  EXPECT_FALSE(pattern.Matches(GURL("http://subdomain.nixbrowser.in")));
  EXPECT_FALSE(pattern.Matches(GURL("http://brave2.com")));

  // path is a wildcard
  pattern = GetPatternFromURL(GURL("http://nixbrowser.in/path1"));
  EXPECT_TRUE(pattern.Matches(GURL("http://nixbrowser.in")));
  EXPECT_TRUE(pattern.Matches(GURL("http://nixbrowser.in/path1")));
  EXPECT_TRUE(pattern.Matches(GURL("http://nixbrowser.in/path2")));
  EXPECT_FALSE(pattern.Matches(GURL("http://subdomain.nixbrowser.in")));
  ...
  // port is a wildcard
  pattern = GetPatternFromURL(GURL("http://nixbrowser.in:8080"));
  EXPECT_TRUE(pattern.Matches(GURL("http://nixbrowser.in")));
  EXPECT_TRUE(pattern.Matches(GURL("http://nixbrowser.in:8080")));
  EXPECT_TRUE(pattern.Matches(GURL("http://nixbrowser.in:8080/path1")));
  EXPECT_TRUE(pattern.Matches(GURL("http://nixbrowser.in:8080/path2")));
  EXPECT_TRUE(pattern.Matches(GURL("http://nixbrowser.in:5555")));
  EXPECT_TRUE(pattern.Matches(GURL("https://nixbrowser.in")));
  EXPECT_TRUE(pattern.Matches(GURL("https://nixbrowser.in:8080")));
  EXPECT_FALSE(pattern.Matches(GURL("http://subdomain.nixbrowser.in")));
  EXPECT_FALSE(pattern.Matches(GURL("http://brave2.com")));
...
TEST_F(BraveShieldsUtilTest, SetBraveShieldsEnabled_ForOrigin) {
  ...
  brave_shields::SetBraveShieldsEnabled(map, false, GURL("http://nixbrowser.in"));
  // setting should apply to origin
  auto setting = map->GetContentSetting(GURL("http://nixbrowser.in"), GURL(),
                                        ContentSettingsType::BRAVE_SHIELDS);
  EXPECT_EQ(CONTENT_SETTING_BLOCK, setting);

  // setting should apply to different scheme
  setting = map->GetContentSetting(GURL("https://nixbrowser.in"), GURL(),
                                   ContentSettingsType::BRAVE_SHIELDS);
  EXPECT_EQ(CONTENT_SETTING_BLOCK, setting);
...
TEST_F(BraveShieldsUtilTest, GetBraveShieldsEnabled_ForOrigin) {
  ...
  setting =
      brave_shields::GetBraveShieldsEnabled(map, GURL("http://nixbrowser.in"));
  EXPECT_EQ(true, setting);
  setting =
      brave_shields::GetBraveShieldsEnabled(map, GURL("https://nixbrowser.in"));
  EXPECT_EQ(true, setting);
...
TEST_F(BraveShieldsUtilTest, SetAdControlType_Default) {
  ...
  EXPECT_EQ(CONTENT_SETTING_BLOCK, setting);
  setting = map->GetContentSetting(GURL("http://nixbrowser.in"), GURL(),
...
TEST_F(BraveShieldsUtilTest, GetAdControlType_ForOrigin) {
  ...
  auto setting = brave_shields::GetAdControlType(map, GURL());
  EXPECT_EQ(ControlType::BLOCK, setting);
  setting = brave_shields::GetAdControlType(map, GURL("http://nixbrowser.in"));
  EXPECT_EQ(ControlType::BLOCK, setting);
  setting = brave_shields::GetAdControlType(map, GURL("https://nixbrowser.in"));
  EXPECT_EQ(ControlType::BLOCK, setting);
...
TEST_F(BraveShieldsUtilTest, GetAdControlType_ManagedPref) {
  ...
  EXPECT_EQ(ControlType::ALLOW,
            brave_shields::GetAdControlType(map, GURL("http://nixbrowser.in")));
...
TEST_F(BraveShieldsUtilTest, SetCookieControlType_Default) {
  ...
  setting =
      map->GetContentSetting(GURL("http://nixbrowser.in"), GURL("http://nixbrowser.in"),
                             ContentSettingsType::COOKIES);
  EXPECT_EQ(CONTENT_SETTING_ALLOW, setting);
  ...
  setting = map->GetContentSetting(GURL("http://nixbrowser.in"), GURL(),
                                   ContentSettingsType::COOKIES);
  EXPECT_EQ(CONTENT_SETTING_BLOCK, setting);
  setting = map->GetContentSetting(GURL("http://nixbrowser.in"),
...
TEST_F(BraveShieldsUtilTest, SetCookieControlType_ForOrigin) {
  ...
  brave_shields::SetCookieControlType(
      map, profile()->GetPrefs(), ControlType::ALLOW, GURL("http://nixbrowser.in"));
...
TEST_F(BraveShieldsUtilTest, GetCookieControlType_WithUserSettings) {
  ...
  auto cookies_settings =
      get_effective_cookie_state(cookies.get(), GURL("http://nixbrowser.in"));
  ...
  map->SetContentSettingCustomScope(
      ...
      ContentSettingsPattern::FromURL(GURL("http://nixbrowser.in")),
...
TEST_F(BraveShieldsUtilTest, GetCookieControlType_ForOrigin) {
  ...
  /* BLOCK */
  brave_shields::SetCookieControlType(
      map, profile()->GetPrefs(), ControlType::BLOCK, GURL("http://nixbrowser.in"));
  setting = brave_shields::GetCookieControlType(map, cookies.get(),
                                                GURL("http://nixbrowser.in"));
  EXPECT_EQ(ControlType::BLOCK, setting);
  setting = brave_shields::GetCookieControlType(map, cookies.get(), GURL());
...
TEST_F(BraveShieldsUtilTest, GetHttpsUpgradeControlType_ManagedPref) {
  ...
  EXPECT_EQ(
      ControlType::BLOCK_THIRD_PARTY,
      brave_shields::GetHttpsUpgradeControlType(map, GURL("http://nixbrowser.in")));
}
...
TEST_F(BraveShieldsUtilTest, GetNoScriptControlType_ForOrigin) {
  ...
  EXPECT_EQ(ControlType::ALLOW, setting);
  setting =
      brave_shields::GetNoScriptControlType(map, GURL("http://nixbrowser.in"));
  EXPECT_EQ(ControlType::ALLOW, setting);
  setting =
      brave_shields::GetNoScriptControlType(map, GURL("https://nixbrowser.in"));
  EXPECT_EQ(ControlType::ALLOW, setting);
...
TEST_F(BraveShieldsUtilDomainBlockFeatureTest, GetDomainBlockingType) {
  ExpectDomainBlockingType(GURL("https://nixbrowser.in"),
...
TEST_F(BraveShieldsUtilTest, GetDomainBlockingType_ControlTypes) {
  auto* map = HostContentSettingsMapFactory::GetForProfile(profile());
  const GURL url = GURL("https://nixbrowser.in");
